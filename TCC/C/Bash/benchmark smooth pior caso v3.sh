...
# =============================================================
# Benchmark Smoothsort v3 — Pior Caso
# Arquivo único: dados1M_smooth_pior.txt (1M valores)
# Tamanhos: 100k, 200k, 500k, 1M
# =============================================================

ENTRADA="dados1M_smooth_pior.txt"
PROGRAMA="./smoothsort_v3"
REPETICOES=10
CSV_TEMPO="metricas_smooth_pior.csv"
CSV_ENERGIA="energia_smooth_pior.csv"
...
for QUANTIDADE in 100000 200000 500000 1000000; do
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        /usr/bin/time -v $PROGRAMA "$ENTRADA" "saida_smooth_pior.txt" "$QUANTIDADE" 2> "$TMP_TIME"
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        sudo perf stat -e power/energy-pkg/,power_core/energy-core/ \
            $PROGRAMA "$ENTRADA" "saida_smooth_pior.txt" "$QUANTIDADE" 2> "$TMP_PERF"
