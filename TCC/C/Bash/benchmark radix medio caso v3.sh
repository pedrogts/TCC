...
# =============================================================
# Benchmark Radix Sort v3 — Medio Caso
# Arquivo único: dados1M_radix_medio.txt (1M valores)
...
REPETICOES=10
CSV_TEMPO="metricas_radix_medio.csv"
CSV_ENERGIA="energia_radix_medio.csv"
...
for QUANTIDADE in 100000 200000 500000 1000000; do
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        /usr/bin/time -v $PROGRAMA "$ENTRADA" "saida_radix_medio.txt" "$QUANTIDADE" 2> "$TMP_TIME"
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        sudo perf stat -e power/energy-pkg/,power_core/energy-core/ \
            $PROGRAMA "$ENTRADA" "saida_radix_medio.txt" "$QUANTIDADE" 2> "$TMP_PERF"
