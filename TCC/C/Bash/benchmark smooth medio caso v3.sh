...
# =============================================================
# Benchmark Smoothsort v3 — Medio Caso
# Arquivo único: dados1M_smooth_medio.txt (1M valores)
# Tamanhos: 100k, 200k, 500k, 1M
# =============================================================

ENTRADA="dados1M_smooth_medio.txt"
PROGRAMA="./smoothsort_v3"
REPETICOES=10
CSV_TEMPO="metricas_smooth_medio.csv"
CSV_ENERGIA="energia_smooth_medio.csv"
...
for QUANTIDADE in 100000 200000 500000 1000000; do
    echo "=============================="
    echo "Rodando $PROGRAMA — MEDIO CASO — $QUANTIDADE números"
    echo "=============================="
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        /usr/bin/time -v $PROGRAMA "$ENTRADA" "saida_smooth_medio.txt" "$QUANTIDADE" 2> "$TMP_TIME"
        ...
        echo "$PROGRAMA,medio,$i,$QUANTIDADE,$TEMPO_SORT,$TEMPO_USER,$TEMPO_SYS,$CPU,$TEMPO_DEC,$MEM,$FALHAS,$TROCAS_V,$TROCAS_I" >> "$CSV_TEMPO"
        rm -f "$TMP_TIME"
    done
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        sudo perf stat -e power/energy-pkg/,power_core/energy-core/ \
            $PROGRAMA "$ENTRADA" "saida_smooth_medio.txt" "$QUANTIDADE" 2> "$TMP_PERF"

        ENERGIA_PKG=$(extrair_perf_energia "$TMP_PERF" "energy-pkg")
        ENERGIA_CORE=$(extrair_perf_energia "$TMP_PERF" "energy-core")

        echo "$PROGRAMA,medio,$i,$QUANTIDADE,$ENERGIA_PKG,$ENERGIA_CORE" >> "$CSV_ENERGIA"
        rm -f "$TMP_PERF"
    done
done
