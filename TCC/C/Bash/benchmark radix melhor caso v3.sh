...
# =============================================================
# Benchmark Radix Sort v3 — Melhor Caso
# Arquivo único: dados1M_radix_melhor.txt (1M valores)
# Tamanhos: 100k, 200k, 500k, 1M
# =============================================================

ENTRADA="dados1M_radix_melhor.txt"
PROGRAMA="./radix_sort_v3"
REPETICOES=10
CSV_TEMPO="metricas_radix_melhor.csv"
CSV_ENERGIA="energia_radix_melhor.csv"
...
for QUANTIDADE in 100000 200000 500000 1000000; do
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        /usr/bin/time -v $PROGRAMA "$ENTRADA" "saida_radix_melhor.txt" "$QUANTIDADE" 2> "$TMP_TIME"
    ...
    for i in $(seq 1 $REPETICOES); do
        ...
        sudo perf stat -e power/energy-pkg/,power_core/energy-core/ \
            $PROGRAMA "$ENTRADA" "saida_radix_melhor.txt" "$QUANTIDADE" 2> "$TMP_PERF"
